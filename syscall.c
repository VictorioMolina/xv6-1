...
int fetchint(uint addr, int *ip) {
	...
	//dont check on init
	if (cmpinit(curproc) == 0) {

		if (addr >= curproc->stacklastpg && addr < curproc->guardpg + PGSIZE) {
			return -1;
		}
		if (addr + 4 >= curproc->shmbreak && addr < curproc->shmtop) {
			return -1;
		}
	}
...
int fetchstr(uint addr, char **pp) {
	...
	if (cmpinit(curproc) == 0) {
		...
		if (addr >= curproc->shmbreak && addr < curproc->shmtop) {
			return -1;
		}
	}
...
int argptr(int n, char **pp, int size) {
	...
	if (cmpinit(curproc) == 0) {
		...
		if ((uint) i >= curproc->stacklastpg && (uint) i < curproc->guardpg + PGSIZE) {
			return -1;
		}
		if ((uint)( i + size) >= curproc->shmbreak
				&& (uint) i < curproc->shmtop) {
			return -1;
		}
	}
...
extern int sys_futex_wait2(void);
...
static int (*syscalls[])(void) = {
	...
	[SYS_futex_wait2] sys_futex_wait2,
};
